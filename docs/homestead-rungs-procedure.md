...
| I-31 | The cover's resting state reveals nothing — ~~a count is shown only after it clears the k≥2 re-identification check on both the count and the matters.~~ (struck 2026-09-11, widened by E4; the rule's direction is unchanged, its second gate is stronger) a count is shown only after it clears the k≥2 check on the count **and** on the *distinct* matters — the roster is read as a set — and, when the caller states the per-matter distribution, only when ≥K distinct matters each contribute ≥1 to that category, so `(2,0)` is dropped where `(1,1)` passes. | `homestead/app/cover.py:1-15`, `docs/DECISION-cover-re-identification.md` |
