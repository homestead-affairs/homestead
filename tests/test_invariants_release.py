...
Ported into homestead from jeles and willow-mcp, where those four were paid for.
This is the engine's first PyPI wiring, so these are guards against a first-time
miswire and a future edit, not a live bug.

**Ported, not copied.** The checks that differ by repo are made to fail if a
constant was carried over unchanged: homestead is below 1.0 and packages
`homestead/`, not willow-mcp's `src/willow_mcp/`; and it stores no version
literal anywhere, because the tag is the only source. Getting those backwards is
exactly the copy-paste failure the wiring test exists to catch.
"""
...
def test_the_version_has_exactly_one_source():
    ...
    drift. `homestead/__init__.py` carried `__version__ = "0.0.1"` before this
    wiring landed, which is exactly the drift being closed."""
    ...
    hardcoded = [
        ...
        for p in (_REPO / "homestead").rglob("*.py")
...
def test_the_pr_title_check_guards_both_directions():
    ...
    layout and `jeles/` is jeles'; here the wheel packages `homestead`, so a
    copied constant would make the check pass on everything."""
    wf = _REPO / ".github" / "workflows" / "pr-title.yml"
    body = _yaml(wf)["jobs"]["title"]["steps"][-1]["run"]
    assert 'PACKAGED = ("homestead/", "pyproject.toml")' in body, \
    ...
    pyproject = tomllib.loads((_REPO / "pyproject.toml").read_text())
    assert pyproject["tool"]["hatch"]["build"]["targets"]["wheel"]["packages"] == ["homestead"]
...
def test_a_breaking_change_below_1_0_cuts_1_0_0_rather_than_a_minor():
    ...
    produce. homestead-law depends on this package and pins it, so the range must
    mean something: 1.0 arrives when compatibility breaks, not when someone
    declares it."""
    cfg = _package_config()
