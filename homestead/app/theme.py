...
**The shared surface theme.** It was proven in `homestead-ledger`'s bite 3 and
hoisted here so both `homestead-law` and `homestead-ledger` draw from one copy
rather than each keeping its own — the fleet's rediscovery tax, paid once. A view
in either module calls `homestead.app.theme.apply(root)` and colours its list
rows with `rung_color`; there is no second copy to drift. It depends only on the
rung enum (`homestead.keep.rungs`), so it sits in the engine every module already
pins.
